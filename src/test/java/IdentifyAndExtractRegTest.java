...
//                "问：找谁快递！元什么是退货快递单号，怎么上传？间：退货须知请确保商品不影响二水销售（质量问题除外）撤销售后";
        content = "退货退款详情请选择退货方式，退回商品请在6天23小时内填写快递单号昆ro上门取件自行退回去服务点去快递柜最快2小时并填写单号随到随退全天可退官方退货|免填单号丢损赔偿上门时间2小时上门取件地址公交车家的关关15698894334辽宁省大连市沙河口区高尔基路320号公交308室运营商家地址贺琦【多多售后】18062758083湖北省武汉市黄陂区湖北省武汉0C市黄陂区汉口北大道华中企业城E";
...
//        pattern = Pattern.compile("(1[3-9]\\d{9}|\\d{3}-\\d{3}-\\d{4}|\\d{3,4}[ -]?\\d{7,8}).*?(1[3-9]\\d{9}|\\d{3}-\\d{3}-\\d{4}|\\d{3,4}[ -]?\\d{7,8})(.*?)(?=(退件方式|上门取件|去快递柜|去服务点))");
        ...
        pattern = Pattern.compile(".*(退货|商家)地址：?(.*?)(1[3-9]\\d{9}|\\d{3,4}[ -]?\\d{7,8}|\\d{3}-\\d{3}-\\d{4}).*");
        pattern = Pattern.compile(".*(退货|商家)地址：?(.*?)(1[3-9]\\d{9}|\\d{3,4}[ -]?\\d{7,8}|\\d{3}-\\d{3}-\\d{4}).*");
