...
1. 扫描应用内部打了上`AbilityDeclare` 注解的AbilitySPI实现类。保存能力名称->能力实例的映射关系(map)
2. 扫描应用内部打上`TdcqfAbilityAgent` 注解的BaseAbilityAgent子类，作为HSF服务发布出去
* 接口名:AbilityAgent
* 版本号:1.0.0.env.appname  eg:1.0.0.daily.tdfulfil

#### 2.应用之间调用的方式
1. 调用源头来源于磐石:磐石判断出该能力的代理应用是哪一个(比如交易自己代理自己，实操的能力由履行代理。因为磐石不感知业务，履行为实操构造所需要的数据)。 
2. 如果是自己本身的能力则直接调用，如果是别的应用的能力，则调用该应用的Agent
