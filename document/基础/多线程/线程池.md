...
1. 当线程池中的线程数量小于corePoolSize，新提交任务将创建一个新线程执行任务，即使此时线程池中存在空闲线程。
...
4. 当提交任务数量超过 maximumPoolSize 时，新提交任务由 RejectedExecutionHandler 处理。
5. 当线程池中线程数据超过corePoolSize 线程，空闲时间达到 keepAliveTime 时，关闭空闲线程。
...
CPU 密集型任务尽可能的少的核心线程数量，一般为 CPU 核数 + 1 个线程的线程池。
