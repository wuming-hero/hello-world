...
####  当前读（Consistent Read）
* 阻塞：会读取最新的、已经提交的数据版本。如果其他事务对同一行数据进行了修改但还未提交，当前读会`等待这些事务完成或使用锁机制确保读取到的是最新提交的数据`

当前读操作会加锁（通常是行级锁），保证读取的数据是事务开始时或当前时刻的真实状态。
例如，`SELECT ... FOR UPDATE` 或 `SELECT ... LOCK IN SHARE MODE` 操作会使用当前读。
在读取时，它会对数据加锁，以保证读取到的是最新的值，并且不会受到其他事务修改的影响。

#### 快照读（Snapshot Read）：
快照读是指在事务开始时，读取的数据是事务开始时的一致快照，不会被其他事务的更新所影响。
* 非阻塞：快照读不需要加锁，因此不会阻塞其他事务的操作,它通过 MVCC（多版本并发控制）机制实现，能够让事务在读取数据时看到一个一致的历史视图。
快照读确保了事务的隔离性，使得一个事务在执行期间始终看到一致的数据视图，而不受其他事务的影响。

##### 快照读与隔离级别
快照读通常在 READ COMMITTED、REPEATABLE READ 和 SERIALIZABLE 隔离级别下使用，特别是在 REPEATABLE READ 隔离级别下，快照读是默认的行为。此级别使用快照读来读取数据，从而避免了脏读和不可重复读的问题，并确保事务内的数据一致性。
* READ COMMITTED：每次读取数据都会看到事务提交时的数据快照。
* REPEATABLE READ：确保整个事务过程中读取到的数据快照一致，不会被其他事务修改。
...
### 4. 其他优化手段除了慢 SQL 定位、索引优化和架构优化，还可以提及一些其他优化手段，展示你对 MySQL 性能调优的全面理解：
