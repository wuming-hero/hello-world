...
## 1. Seata-AT(Automatic Transaction)模式
...
通信流程：
* 发现：TC 注册到 Nacos/Eureka 等；各服务的 Seata Client 发现并连上 TC。
* 长连接：一般是 Netty 双向长连接（Seata 自有协议），不是业务 HTTP/Dubbo。
* 谁主动：
  * 一阶段：RM → TC（注册分支、申请全局锁、上报分支成功/失败）
  * 二阶段：TC → RM（对该连接下发 branch commit / branch rollback）

所以是「每个 RPC 服务进程里的 RM」一直挂在 TC 上，TC 按 XID + branchId 通知对应 RM，而不是按业务 URL 找服务。
业务 RPC 只传 XID；真正的协调是 各服务内的 Seata Client 与 TC 的长连接。TC 协调的是「事务分支」，不是「业务 RPC 调用链」。

全局锁机制‌：

防止并发事务写同一行数据导致脏写。在本地事务提交前需获取全局锁，若获取失败则重试或超时回滚。

数据源代理‌：

使用 DataSourceProxy 包装原始数据源，拦截 SQL，自动生成前后镜像并写入 undo_log 表。



### 必备条件
* 数据库支持本地 ACID 事务（如 MySQL、PostgreSQL、Oracle 等）。
* Java 应用通过 JDBC 访问数据库。
* 每个参与事务的数据库中需存在 undo_log 表。
* 使用 Seata 提供的 DataSourceProxy 代理数据源。

### 适用场景
✅ ‌推荐使用‌：
* 简单 CRUD 操作（如订单创建、库存扣减）。
* Java 技术栈（Spring Cloud/Dubbo）。
* 并发不高、SQL 结构简单（单表更新、主键/唯一索引条件）。

❌ ‌不推荐使用‌：
* 复杂 SQL（如子查询、JOIN、批量更新、非确定性函数如 NOW()）。
* 高并发写场景（全局锁会成为瓶颈）。
* 跨语言微服务（仅 Java 支持）。
* 长事务（长时间持有全局锁影响性能）。
...
### 常见问题与最佳实践
* Undo Log 表膨胀‌：因全局事务未及时提交/回滚，导致 undo_log 积压。需监控并定期清理。
* 全局锁冲突‌：高并发下频繁重试，可调整 lockRetryInterval 和 lockRetryTimes 。
* 不兼容 SQL‌：如 UPDATE ... SET balance = balance - x，因无法生成确定性 undo，应改用 TCC。
* 生产部署‌：Seata Server（TC）建议使用 db 模式 + Nacos 注册中心，确保高可用 。
