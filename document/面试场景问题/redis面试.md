...
以下为 **Redis Cluster** 在 **reshard / 槽迁移** 过程中，客户端常见疑问的问答形式整理，便于与上文「数据不均、扩缩容迁移」主题对照阅读。

## Q1：客户端访问一个 key，该 key 正在迁往另一节点，且目标节点上数据尚未落全、还不能稳定提供服务，这时会怎样？

**答：** 协议不会卡在「两边都答不上」的状态，而是用 **MOVED / ASK**（及 **ASKING**）把请求导到**当前仍持有数据**或**已迁入数据**的节点上；应由 **支持 Cluster 的客户端** 自动重试，自研路由则需实现相同语义。

| 客户端先打到谁 | 典型行为（简述） |
|----------------|------------------|
| **源节点 A**（槽仍按稳定拓扑归 A 管） | key **还在 A**：A **正常返回**；key **已迁到 B**：A 返回 **ASK → B**，客户端对 B 先发 **ASKING** 再发原命令。 |
| **目标节点 B**（迁入中，数据可能未到齐） | **未带 ASKING**：B 往往返回 **MOVED**，把客户端指回槽的**当前负责主**（多数仍为 **A**）。**已带 ASKING**（跟 ASK 流程）：若 key **尚未**到 B，规范上可再 **ASK 指回仍持有数据的 A**，直到该 key 迁完；迁完后在 B 上正常命中或再经 **MOVED** 稳定拓扑。 |

**小结：** 迁移中间态下，**数据在哪台，就由哪台答，或通过 ASK/MOVED 把客户端指过去**；不会在「空的 B」上静默失败，而是明确重定向。

---

## Q2：什么时候返回 **ASK**，什么时候返回 **MOVED**？二者区别是什么？

**答：**

| 返回 | 什么时候出现 | 对客户端的含义 |
|------|----------------|----------------|
| **MOVED** | 集群里该 **槽的归属已稳定**，你当前连的节点**不负责**这个槽（或本地槽映射过期）。 | **更新槽 → 节点映射**，之后同一槽的请求**直接**打新负责节点。 |
| **ASK** | 槽处于 **MIGRATING / IMPORTING** 等**迁移中**，「稳定槽表」与「数据实际所在」可能暂时不一致。 | **不要**仅凭 ASK 永久改写整槽路由；对目标节点需 **ASKING + 原命令** 做**本次**重试，最终以 **MOVED** / `CLUSTER SLOTS` 等为准。 |

**一句话区别：** **MOVED = 路由表该改了；ASK = 迁移未结束，这次先去指定节点试（常配合 ASKING），不等同于最终拓扑。**

---

## Q3（补充）：使用上有什么注意点？

**答：** 生产环境务必使用 **Cluster 感知客户端**（自动处理 MOVED、ASK、ASKING、重试与退避）；迁移窗口期避免手写「只连一个节点、不处理重定向」的逻辑，否则易出现异常或旧拓扑下的错误节点访问。
